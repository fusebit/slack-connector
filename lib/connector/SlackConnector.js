...
class SlackConnector extends OAuthConnector {
  ...
  onCreate(app) {
    ...
    // Send notification to a Slack user identified with Slack user ID, or with foreign vendor ID and foreign user ID
    app.post(
  ...
  async getAuthorizationUrl(fusebitContext, state, redirectUri) {
    return [
      ...
      fusebitContext.configuration.slack_scope
        ? `&scope=${encodeURIComponent(fusebitContext.configuration.slack_scope)}`
        : undefined,
  ...
  async getHealth(fusebitContext, userContext) {
    const client = await this.createSlackClient(fusebitContext, userContext);
    try {
      await client.bot.api.test();
    } catch (e) {
